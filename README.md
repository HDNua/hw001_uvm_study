...
UART TX를 시작으로 RTL 검증 구조를 단계별로 확장하는 학습용 프로젝트입니다. 직접 구동·검사에서 역할 분리와 queue handoff를 거쳐, `m11_mailbox_channel`에서는 sequence item 전달 경로를 typed SystemVerilog mailbox로 구성합니다.
...
저장소 루트에서 실행할 단계의 `sim` 디렉터리로 이동합니다. 예를 들어 `m11_mailbox_channel`은 다음과 같이 실행합니다.

```powershell
cd .\260329_uart\m1_uart_tx\m11_mailbox_channel\sim
.\run_xsim.ps1
```
...
성공하면 m11 로그에 smoke·pattern·random 세 case, sequencer mailbox item put/get과 sequence·driver·monitor 처리 각 15건, scoreboard PASS 15건이 출력됩니다. 각 case는 `[SB] RESULT: pass=5 fail=0`으로 끝납니다. 파형 데이터와 로그를 포함한 모든 시뮬레이션 산출물은 `sim/out/`에 생성되며 Git에서 제외됩니다.
