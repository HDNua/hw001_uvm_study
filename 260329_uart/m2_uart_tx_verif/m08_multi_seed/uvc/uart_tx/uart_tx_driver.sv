...
class uart_tx_driver extends uvm_driver #(uart_tx_seq_item);
    ...
    // 실제 구동한 request item을 coverage 같은 관찰자에게 publish한다.
    uvm_analysis_port #(uart_tx_seq_item) req_ap;
    ...
    task drive_item(input uart_tx_seq_item item);
        ...
        // 전송 중 리셋 대비: 리셋 해제와 ready를 함께 기다린 뒤 구동한다.
        do @(posedge vif.i_Clk); while (!(vif.w_TxReady === 1'b1 && vif.i_Rsn === 1'b1));
