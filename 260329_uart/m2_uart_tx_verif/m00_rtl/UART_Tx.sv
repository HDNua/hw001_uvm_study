...
//
// -----------------------------------------------------------------------------
// 핸드셰이크 규약 (i_TxValid / o_TxReady)
// -----------------------------------------------------------------------------
//   - o_TxReady=1 (IDLE)일 때 접수된 i_TxValid만 전송으로 이어진다.
//   - o_TxReady=0 (전송 중)일 때의 i_TxValid는 조용히 무시된다.
//     내부 큐잉과 에러 신호는 없으며 해당 i_TxData는 유실된다.
//   - 따라서 호스트는 o_TxReady=1을 확인한 뒤에만 요청해야 한다.
//   - 이 규약은 m2_uart_tx_verif/m03_corner의 busy 중 valid 주입
//     시나리오로 검증된다.
//
// -----------------------------------------------------------------------------
// 파라미터
