// 실제 UVM test.
// 4개 case에 전송 중 리셋 case를 더해 복구 경로까지 검증한다.
...
class uart_tx_test extends uvm_test;
    ...
    localparam int UART_TX_SEQ_RESET   = 4;
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (num_bytes < 2)
            `uvm_fatal("CFG", "NUM_BYTES must be >= 2 for the mid-reset case")
    ...
    task run_phase(uvm_phase phase);
        ...
        run_case(UART_TX_SEQ_RESET);

        `uvm_info("TEST", "UART_TX_VERIF_ALL_DONE cases=5", UVM_LOW)
    ...
    task run_case(input int seq_kind);
        ...
        case (seq_kind)
            ...
            UART_TX_SEQ_RESET: begin
                // 전송 자체는 random 자극을 쓰고 리셋은 test가 주입한다.
                seq.set_random_payload(sequence_name, num_bytes);
            end
            default: begin
                print_payload();
        ...
        if (seq_kind == UART_TX_SEQ_RESET) begin
            fork
                seq.start(env.agent.sequencer);
                inject_mid_frame_reset();
            join
        end else begin
            seq.start(env.agent.sequencer);
        end
    ...
    // 두 번째 프레임의 중간에서 리셋을 주입한다.
    // 잘린 byte는 monitor가 버리고 scoreboard가 목표에서 제외한다.
    task inject_mid_frame_reset();
        repeat (2) @(negedge vif.w_TxReady);
        repeat ($urandom_range(CLKS_PER_BIT, 8 * CLKS_PER_BIT)) @(posedge vif.i_Clk);

        `uvm_info("TEST", "UART_TX_VERIF_RESET_INJECTED", UVM_LOW)
        vif.r_RsnDrive = 1'b0;
        env.scoreboard.on_reset();
