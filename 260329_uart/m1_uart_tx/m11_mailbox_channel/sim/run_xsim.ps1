...
$TbFile        = (Resolve-Path (Join-Path $StageDir "tb\top\tb_top_v11.sv")).Path
...
$ExpectedTestDone = "[TEST] MAILBOX_CHANNEL_ALL_DONE cases=3"
...
"@ | Set-Content -LiteralPath $RunTcl -Encoding ASCII
    ...
    $CaseStartCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[TEST\] MAILBOX_CHANNEL_CASE_START').Count
    if ($CaseStartCount -ne $ExpectedCaseCount) {
        throw "Expected $ExpectedCaseCount case starts, found $CaseStartCount in $SimLog"
    }
    $CaseDoneCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[TEST\] MAILBOX_CHANNEL_CASE_DONE').Count
    ...
    $SequenceCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[SEQ\] mailed item/expected:').Count
