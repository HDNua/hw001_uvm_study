...
// m10과 같은 smoke, pattern, random case를 mailbox 기반 UART TX UVC에서 실행한다.
...
task automatic uart_tx_run_case(input int seq_kind);
    ...
    $display("[TEST] MAILBOX_CHANNEL_CASE_START kind=%s bytes=%0d", case_name, UART_TX_NUM_BYTES);
    ...
    $display("[TEST] MAILBOX_CHANNEL_CASE_DONE kind=%s", case_name);
...
task automatic uart_tx_test();
    ...
    $display("[TEST] MAILBOX_CHANNEL_ALL_DONE cases=3");
endtask
