// UART TX test class.
//
// m13의 request 경로를 유지하고 monitor-scoreboard actual 경로에 analysis port를 연결한다.
...
class uart_tx_test;
    ...
    function new(virtual uart_tx_if vif);
        ...
        env           = new(vif, seq_item_port, exp_mbx);
    endfunction
    ...
    task run_case(input int seq_kind);
        ...
        sequencer.reset();
        env.scoreboard.reset(UART_TX_NUM_BYTES);

        $display("[TEST] SV_ANALYSIS_PORT_CASE_START kind=%s bytes=%0d", case_name, UART_TX_NUM_BYTES);
        print_payload();
        ...
        $display("[TEST] SV_ANALYSIS_PORT_CASE_DONE kind=%s", case_name);
    ...
    task run();
        ...
        $display("[TEST] SV_ANALYSIS_PORT_ALL_DONE cases=3");
    endtask
endclass
