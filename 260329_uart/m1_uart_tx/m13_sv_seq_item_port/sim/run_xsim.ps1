...
$TbFile        = (Resolve-Path (Join-Path $StageDir "tb\top\tb_top_v13.sv")).Path
...
$ExpectedBuildDone = "[TEST] SV_SEQ_ITEM_PORT_OBJECTS_CREATED"
$ExpectedTestDone = "[TEST] SV_SEQ_ITEM_PORT_ALL_DONE cases=3"
...
"@ | Set-Content -LiteralPath $RunTcl -Encoding ASCII
    ...
    if (!(Select-String -Quiet -SimpleMatch -LiteralPath $SimLog -Pattern $ExpectedBuildDone)) {
        throw "Expected seq_item_port object creation marker not found: $ExpectedBuildDone"
    }
    $CaseStartCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[TEST\] SV_SEQ_ITEM_PORT_CASE_START').Count
    if ($CaseStartCount -ne $ExpectedCaseCount) {
        throw "Expected $ExpectedCaseCount case starts, found $CaseStartCount in $SimLog"
    }
    $CaseDoneCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[TEST\] SV_SEQ_ITEM_PORT_CASE_DONE').Count
    ...
    if ($PortGetCount -ne $ExpectedItemCount) {
        throw "Expected $ExpectedItemCount port get_next_item results, found $PortGetCount in $SimLog"
    }
    $PortDoneCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[PORT\] item_done').Count
    if ($PortDoneCount -ne $ExpectedItemCount) {
        throw "Expected $ExpectedItemCount port item_done results, found $PortDoneCount in $SimLog"
    }
    ...
    if ($SequencerDoneCount -ne $ExpectedItemCount) {
        throw "Expected $ExpectedItemCount sequencer item_done results, found $SequencerDoneCount in $SimLog"
    }
