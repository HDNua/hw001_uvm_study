// UART TX test class.
//
// m12의 class UVC를 유지하면서 driver 앞에 seq_item_port bridge를 연결한다.
...
class uart_tx_test;
    ...
    // test가 생성한 같은 sequencer를 sequence와 port가 공유한다.
    ...
    function new(virtual uart_tx_if vif);
        ...
        sequencer     = new();
        seq_item_port = new(sequencer);
        seq           = new(sequencer, exp_mbx);
        env           = new(vif, seq_item_port, exp_mbx, mon_mbx);
    endfunction
    ...
    task run_case(input int seq_kind);
        ...
        $display("[TEST] SV_SEQ_ITEM_PORT_CASE_START kind=%s bytes=%0d", case_name, UART_TX_NUM_BYTES);
        print_payload();
        ...
        $display("[TEST] SV_SEQ_ITEM_PORT_CASE_DONE kind=%s", case_name);
    ...
    task run();
        ...
        $display("[TEST] SV_SEQ_ITEM_PORT_ALL_DONE cases=3");
    endtask
endclass
