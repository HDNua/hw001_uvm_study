...
$TbFile        = (Resolve-Path (Join-Path $StageDir "tb\top\tb_top_v12.sv")).Path
...
$ExpectedBuildDone = "[TEST] SV_CLASS_UVC_OBJECTS_CREATED"
$ExpectedTestDone = "[TEST] SV_CLASS_UVC_ALL_DONE cases=3"
...
"@ | Set-Content -LiteralPath $RunTcl -Encoding ASCII
    ...
    if (!(Select-String -Quiet -SimpleMatch -LiteralPath $SimLog -Pattern $ExpectedBuildDone)) {
        throw "Expected class object creation marker not found: $ExpectedBuildDone"
    }
    $CaseStartCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[TEST\] SV_CLASS_UVC_CASE_START').Count
    if ($CaseStartCount -ne $ExpectedCaseCount) {
        throw "Expected $ExpectedCaseCount case starts, found $CaseStartCount in $SimLog"
    }
    $CaseDoneCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[TEST\] SV_CLASS_UVC_CASE_DONE').Count
    ...
    $SequencerGetCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[SEQR\] get item:').Count
    ...
    $SequenceCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[SEQ\] class item/expected:').Count
    if ($SequenceCount -ne $ExpectedItemCount) {
        throw "Expected $ExpectedItemCount class sequence item results, found $SequenceCount in $SimLog"
    }
