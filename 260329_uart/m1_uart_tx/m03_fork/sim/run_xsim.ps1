...
$TbFile   = (Resolve-Path (Join-Path $StageDir "tb\tb_top_v3.sv")).Path
...
"@ | Set-Content -LiteralPath $RunTcl -Encoding ASCII
    ...
    $PassCount = @(Select-String -LiteralPath $SimLog -Pattern '^PASS:').Count
    if ($PassCount -ne $ExpectedPassCount) {
        throw "Expected $ExpectedPassCount PASS results, found $PassCount in $SimLog"
    }
}
finally {
    Pop-Location
