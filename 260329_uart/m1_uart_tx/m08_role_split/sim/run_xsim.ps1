...
$TbFile   = (Resolve-Path (Join-Path $StageDir "tb\tb_top_v8.sv")).Path
...
$ExpectedTestStart = "[TEST] ROLE_SPLIT_START bytes=5"
$ExpectedTestDone = "[TEST] ROLE_SPLIT_DONE"
...
"@ | Set-Content -LiteralPath $RunTcl -Encoding ASCII
    ...
    $SequenceCount = @(Select-String -LiteralPath $SimLog -Pattern '^\[SEQ\] queued stimulus/expected:').Count
    if ($SequenceCount -ne $ExpectedRoleCount) {
        throw "Expected $ExpectedRoleCount sequence distribution results, found $SequenceCount in $SimLog"
    }
